...
int find(torch::Device device, Prepare&& prepare, Forward&& forward, int minBatchSize, int maxBatchsize, float maxTimeMs, Score&& scoreFunction) {
  ...
  auto scorex = [&](auto& x) {
    return scoreFunction(x.latency / x.n, x.size);
  };
  ...
  for (int j = 0; j != 4; ++j) {
    ...
    for (int k = 0; k != 12; ++k) {
      ...
      for (auto i = li.begin();;) {
        ...
        if (to - from > 0) {
          float weight =
              std::min(scorex(i->second), scorex(next->second)) - minweight;
  ...
  for (auto& [k, v] : li) {
    fmt::printf(
        ...
        v.n, v.latency / v.n, v.size / (v.latency / v.n), scorex(v));
  }
  ...
  fmt::printf("Found best batch size of %d with evals %d latency %fms "
              ...
              li[best].size / (li[best].latency / li[best].n), scorex(li[best]));
  return best;
}

template<typename Prepare, typename Forward>
int find(torch::Device device, Prepare&& prepare, Forward&& forward, int minBatchSize, int maxBatchsize, float maxTimeMs) {
  return find(device, std::forward<Prepare>(prepare), std::forward<Forward>(forward), minBatchSize, maxBatchsize, maxTimeMs, defaultScore);
}
